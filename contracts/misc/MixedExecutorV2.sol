...
import {IPythPriceFeed} from "../oracle/interfaces/IPythPriceFeed.sol";
import {IPythAdaptor} from "../oracle/interfaces/IPythAdaptor.sol";
...
contract MixedExecutorV2 is Multicall, Governable {
    ...
    /// @notice The address of pyth price feed
    IPythPriceFeed public immutable pythPriceFeed;
    /// @notice The address of pyth adaptor
    IPythAdaptor public immutable pythAdaptor;
    ...
    constructor(
        ...
        IPythPriceFeed _pythPriceFeed,
        IPythAdaptor _pythAdaptor,
    ...
    /// @param _packedPythPrices Refer to method `updatePriceFeeds` in PythAdaptor
    /// @param _packedPrices The packed values of the market index and priceX96: bit 0-23 represent the market, and
    /// bit 24-183 represent the priceX96
    /// @param _packedMinPublishTimes The packed timestamp of the price update: bit 0-63 represent the min publish
    /// timestamp of the _packedPythPrices, and bit 64-127 represent the update timestamp of the _packedPrices
    /// @param _encodedVaas A hash of a set of vaa data
    function setPriceX96s(
        PackedValue[] calldata _packedPythPrices,
        PackedValue[] calldata _packedPrices,
        PackedValue _packedMinPublishTimes,
        bytes32 _encodedVaas
    ) external virtual onlyExecutor {
        IPythPriceFeed.MarketPrice[] memory marketPrices = new IPythPriceFeed.MarketPrice[](_packedPrices.length);
        uint256 len = _packedPrices.length;
        for (uint256 i; i < len; ++i) {
            marketPrices[i] = IPythPriceFeed.MarketPrice({
                market: marketIndexer.indexMarkets(_packedPrices[i].unpackUint24(0)),
                priceX96: _packedPrices[i].unpackUint160(24)
            });
        }
        uint64 pythMinPublicTime = _packedMinPublishTimes.unpackUint64(0);
        uint64 minPublishTime = _packedMinPublishTimes.unpackUint64(64);
        bytes32[] memory affectedAssetIds = pythAdaptor.updatePriceFeeds(
            _packedPythPrices,
            pythMinPublicTime,
            _encodedVaas
        );
        pythPriceFeed.setPriceX96s(marketPrices, minPublishTime);
        pythAdaptor.clearPrices(affectedAssetIds);
    }
